...
def savefig(name):
    ...
    latex_fig_dir = "thesis new/latex for our thesis/thesis_figures"
    if os.path.exists(latex_fig_dir):
        import shutil
        dest = os.path.join(latex_fig_dir, f"{name}.{CFG['fig_ext']}")
        shutil.copy(path, dest)
        print(f"  -> Figure copied to LaTeX: {dest}")
...
def plot_three_way_comparison(central_results, fl_results):
    print("  Plotting Figure 5 — Five-way comparison (Linear vs MLR vs Poly Two vs Centralized vs Federated) …")
    plt.style.use(CFG["style"])
    ...
    r2_vals   = [0.8100, 0.8219, 0.8400,
                 central_results["r2"], best_fl["final_r2"]]
    rmse_vals = [8.23, 8.04, 7.38,
                 central_results["rmse"], best_fl["final_rmse"]]

    bar_colors = ["#8E8E8E", "#A6ACAF", "#E07B39", "#2D6A9F", "#3AA66C"]
    ...
    for bar, val in zip(bars, r2_vals):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.01,
                ...
                fontweight="bold")
    # Add improvement arrow for Centralized vs Linear
    ax.annotate("", xy=(3, central_results["r2"]), xytext=(0, 0.8100),
    ...
    plt.suptitle("Figure 5 — Performance Comparison\n"
                 "Linear vs MLR vs Poly Two vs Centralized vs Federated",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
